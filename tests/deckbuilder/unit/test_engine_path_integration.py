...
from src.deckbuilder.engine import Deckbuilder
...
class TestEnginePathManagerIntegration:
    ...
    def test_create_presentation_uses_path_manager_paths(self, mock_exists, mock_makedirs):
        ...
        with patch.object(path_manager, 'get_template_folder') as mock_template, \
            ...
            with patch.object(engine, '_check_template_exists'), \
                 patch.object(engine, '_load_layout_mapping'), \
                 patch.object(engine, 'prs') as mock_prs:
                ...
                # Should create output in PathManager output folder
    ...
    def test_singleton_behavior_with_path_manager(self):
        ...
        with patch.object(path_manager, 'get_template_folder', return_value=Path("/test1")), \
        ...
        with patch.object(path_manager, 'get_template_folder', return_value=Path("/test2")), \
...
class TestEnginePathResolution:
    ...
    def test_check_template_exists_uses_resolved_paths(self, mock_exists):
        ...
        with patch.object(path_manager, 'get_template_folder', return_value=Path("/templates")), \
            ...
            with patch('src.deckbuilder.engine.os.makedirs'), \
                ...
                expected_calls = [
                    (("/templates/custom.pptx",),),
                ]
                # At least one call should be to the resolved template path
                assert any("/templates/custom.pptx" in str(call) for call in mock_exists.call_args_list)
    ...
    def test_load_layout_mapping_uses_resolved_paths(self):
        ...
        with patch.object(path_manager, 'get_template_folder', return_value=Path("/templates")), \
            ...
            with patch('src.deckbuilder.engine.os.path.exists', return_value=True), \
