...
class TestSemanticFieldMapping:
    ...
    def test_template_has_semantic_two_content_fields(self):
        ...
        for placeholder_name in placeholders.values():
            assert (
                placeholder_name not in confusing_fields
            ), f"Found confusing field name: {placeholder_name}"
            ...
            for placeholder_name in placeholders.values():
                ...
                if placeholder_name.startswith("content_"):
                    ...
                    if not placeholder_name.startswith(
                        ("date_", "footer_", "slide_number_", "content_item", "content_1")
                    ):
                        ...
                        if placeholder_name in [
                            ...
                            assert (
                                False
                            ), f"Confusing field name in {layout_name}: {placeholder_name} (should be semantic)"
                ...
                if "content" in placeholder_name:
                    ...
                    is_footer = any(
                        placeholder_name.startswith(pattern) for pattern in footer_patterns
                    )

                    if not is_footer:
                        assert (
                            is_semantic or placeholder_name == "content_1"
                        ), f"Non-semantic content field in {layout_name}: {placeholder_name}"
    ...
    def test_no_placeholder_id_confusion(self):
        ...
        for placeholder_id, expected_name in expected_mapping.items():
            if placeholder_id in placeholders:
                ...
                ), f"Placeholder {placeholder_id} should map to {expected_name}, got {actual_name}"
