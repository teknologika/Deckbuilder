...
class TestActualSlideContentValidation:
    ...
    def _generate_presentation_from_json(self, json_data: dict, filename: str, deckbuilder_with_env, test_output_dir) -> Path:
        ...
        result = deck.create_presentation(presentation_data=canonical_data, fileName=filename, templateName="default")
        ...
        assert len(output_files) > 0, f"No output file generated. Result: {result}. Files in dir: {list(test_output_dir.iterdir())}"
    ...
    def test_simple_json_content_appears_in_slides(self, deckbuilder_with_env, test_output_dir):
        ...
        pptx_path = self._generate_presentation_from_json(json_data, "simple_test", deckbuilder_with_env, test_output_dir)
        validation = self._validate_slide_content(pptx_path, expected_content)

        assert validation["total_slides"] == 2, f"Expected 2 slides, got {validation['total_slides']}"
        assert len(validation["errors"]) == 0, f"Content validation failed: {validation['errors']}"

        for slide_idx in [0, 1]:
            assert validation["slides"][slide_idx]["has_content"], f"Slide {slide_idx + 1} has no content"
            assert len(validation["slides"][slide_idx]["missing_expected"]) == 0, f"Slide {slide_idx + 1} missing expected content"
    ...
    def test_semantic_field_names_content_appears(self, deckbuilder_with_env, test_output_dir):
        ...
        pptx_path = self._generate_presentation_from_json(json_data, "semantic_test", deckbuilder_with_env, test_output_dir)
        validation = self._validate_slide_content(pptx_path, expected_content)

        assert validation["total_slides"] == 1, f"Expected 1 slide, got {validation['total_slides']}"
        assert len(validation["errors"]) == 0, f"Semantic field validation failed: {validation['errors']}"
    ...
    def test_rich_content_blocks_appear_in_slides(self, deckbuilder_with_env, test_output_dir):
        ...
        pptx_path = self._generate_presentation_from_json(json_data, "rich_content_test", deckbuilder_with_env, test_output_dir)
        validation = self._validate_slide_content(pptx_path, expected_content)

        assert validation["total_slides"] == 1, f"Expected 1 slide, got {validation['total_slides']}"
        assert len(validation["errors"]) == 0, f"Rich content validation failed: {validation['errors']}"
        assert validation["slides"][0]["has_content"], "Slide has no content"

    def test_comprehensive_layouts_file_content_appears(self, deckbuilder_with_env, test_output_dir):
        """Test that the comprehensive layouts golden file actually produces content"""
        # Load the actual master layouts file
        golden_json_path = project_root / "src" / "deckbuilder" / "assets" / "master_default_presentation.json"
        with open(golden_json_path, "r") as f:
            json_data = json.load(f)

        try:
            pptx_path = self._generate_presentation_from_json(json_data, "comprehensive_test", deckbuilder_with_env, test_output_dir)
        ...
        for i in range(min(5, len(prs.slides))):
            slide = prs.slides[i]
            has_content = any(shape.text.strip() for shape in slide.shapes if hasattr(shape, "text")) or any(
                paragraph.text.strip() for shape in slide.shapes if hasattr(shape, "text_frame") and shape.text_frame for paragraph in shape.text_frame.paragraphs
            )
