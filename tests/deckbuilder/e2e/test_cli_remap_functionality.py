...
class TestCLIRemapFunctionality:
    ...
    def test_remap_help_command(self):
        ...
        result = subprocess.run(
            ["python", str(Path(__file__).parent.parent.parent.parent / "src" / "deckbuilder" / "cli.py"), "help", "remap"],
            capture_output=True,
            text=True,
        )
    ...
    def test_remap_language_only(self, sample_presentation):
        ...
        backup_files = list(sample_presentation.parent.glob("*.backup.pptx"))
...
class TestRemapBackupValidation:
    ...
    def test_backup_file_structure(self, sample_presentation):
        ...
        original_size = sample_presentation.stat().st_size
