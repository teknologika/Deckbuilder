...
class TestTemplateLayoutSystem:
    ...
    def test_list_available_layouts(self):
        ...
        expected_layouts = [
            ...
            "Picture with Caption", "Table Only", "Table with Content Above",
            "Table with Content Above and Below", "Four Columns", "Three Columns"
        ]
    ...
    def test_validate_layout_exists(self):
        ...
        # Valid layouts
        assert self.layout_resolver.validate_layout_exists(self.mock_prs, "Title and Content") == True
        assert self.layout_resolver.validate_layout_exists(self.mock_prs, "Table Only") == True
        
        # Invalid layout
        assert self.layout_resolver.validate_layout_exists(self.mock_prs, "Invalid Layout") == False
...
class TestPatternLoaderIntegration:
    ...
    def test_pattern_loader_discovers_layouts(self, mock_iterdir, mock_exists):
        ...
        mock_patterns = {
            ...
            "table_only.json": {
                "yaml_pattern": {"layout": "Table Only", "table_data": "table"},
                "validation": {"required_fields": ["table_data"], "optional_fields": []}
            }
        }
    ...
    def test_no_dynamic_shape_methods(self):
        ...
        try:
            ...
            assert not hasattr(slide_builder, '_create_dynamic_content_shapes'), \
                "slide_builder should not have _create_dynamic_content_shapes method"
            
            assert not hasattr(slide_builder, '_content_segments'), \
                "slide_builder should not have _content_segments attribute"
...
class TestTableLayoutsReplacement:
    ...
    def test_mixed_content_uses_layouts(self):
        ...
        for scenario in scenarios:
            assert layout_resolver.validate_layout_exists(mock_prs, scenario), \
                f"Layout '{scenario}' should exist to replace dynamic shape creation"
