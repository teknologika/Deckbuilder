...
class TestSlideCoordinator(unittest.TestCase):
    ...
    def test_dependency_injection_constructor(self):
        ...
        coordinator = SlideCoordinator(
            layout_resolver=mock_layout_resolver,
            placeholder_manager=mock_placeholder_manager,
            content_processor=mock_content_processor,
            table_handler=mock_table_handler
        )
    ...
    def test_lazy_loading_of_dependencies(self):
        ...
        with patch('src.deckbuilder.refactor.layout_resolver.LayoutResolver') as mock_layout_resolver_class:
            with patch('src.deckbuilder.refactor.placeholder_manager.PlaceholderManager') as mock_placeholder_manager_class:
                with patch('src.deckbuilder.core.content_processor.ContentProcessor') as mock_content_processor_class:
                    with patch('src.deckbuilder.core.table_handler.TableHandler') as mock_table_handler_class:
    ...
    def test_create_slide_success_workflow(self, mock_debug):
        ...
        # Create slide
        result = self.coordinator.create_slide(
            self.mock_prs, self.basic_slide_data, self.mock_content_formatter, self.mock_image_handler
        )
        
        # Verify workflow
        assert result == self.mock_slide
        self.coordinator._layout_resolver.resolve_layout_by_name.assert_called_once_with(
            self.mock_prs, "Title and Content"
        )
    ...
    def test_create_slide_invalid_data_type(self):
        ...
        with self.assertRaises(RuntimeError) as context:
            self.coordinator.create_slide(
                self.mock_prs, "invalid_data", self.mock_content_formatter, self.mock_image_handler
            )
    ...
    def test_clear_slides_success(self, mock_debug):
        ...
        self.mock_prs.slides = [mock_slide1, mock_slide2]
    ...
    def test_add_speaker_notes_success(self, mock_debug):
        ...
        self.mock_content_formatter.apply_inline_formatting.assert_called_once_with(
            notes_content, mock_paragraph
        )
        mock_debug.assert_called_with(f"Added speaker notes: {len(notes_content)} characters")

    @patch('src.deckbuilder.core.slide_coordinator.error_print')
    ...
    @patch('src.deckbuilder.refactor.placeholder_normalizer.PlaceholderNormalizer')
    def test_normalize_placeholder_names_success(self, mock_normalizer_class):
        ...
        mock_normalizer.normalize_slide_placeholder_names.assert_called_once_with(
            self.mock_slide, self.mock_slide_layout
        )
    ...
    def test_process_slide_content_workflow(self):
        ...
        self.coordinator._placeholder_manager.map_fields_to_placeholders.return_value = {
        ...
        # Process content
        self.coordinator._process_slide_content(
            self.mock_slide, slide_data, "Title and Content", 
            self.mock_content_formatter, self.mock_image_handler
        )
        ...
        self.coordinator._content_processor.apply_content_to_placeholder.assert_called_once_with(
            self.mock_slide, mock_placeholder, "title", "Test Title", slide_data,
            self.mock_content_formatter, self.mock_image_handler
        )
