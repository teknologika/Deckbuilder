...
def large_table_data():
    ...
    return [[f"Header {i}" for i in range(10)]] + [[f"Row {row}, Col {col}" for col in range(10)] for row in range(100)]
...
def basic_slide_data_simple():
    ...
    return {"layout": "Title and Content", "title": "Test Slide Title", "content": "Test slide content"}
...
def mock_pattern_data():
    ...
    return {
        ...
        "validation": {"required_fields": ["title"], "optional_fields": ["content"], "field_types": {"title": "string", "content": "string"}},
    }
...
def mock_comparison_pattern():
    ...
    return {
        ...
        "yaml_pattern": {"layout": "Comparison", "title": "str", "title_left": "str", "content_left": "str", "title_right": "str", "content_right": "str"},
        "validation": {"required_fields": ["title", "title_left", "content_left", "title_right", "content_right"]},
    }
...
def mock_pattern_loader():
    ...
    loader = Mock()
    loader.get_pattern_for_layout.return_value = {"yaml_pattern": {"layout": "Title and Content", "title": "str", "content": "str"}}
    loader.load_patterns.return_value = {"Title and Content": {"yaml_pattern": {"layout": "Title and Content", "title": "str", "content": "str"}}}
    return loader
...
def assert_table_structure_equal(actual: List[List[str]], expected: List[List[str]]) -> None:
    ...
    for row_idx, (actual_row, expected_row) in enumerate(zip(actual, expected)):
        assert len(actual_row) == len(expected_row), f"Column count mismatch in row {row_idx}: got {len(actual_row)}, expected {len(expected_row)}"

        for col_idx, (actual_cell, expected_cell) in enumerate(zip(actual_row, expected_row)):
            assert actual_cell == expected_cell, f"Cell mismatch at [{row_idx}][{col_idx}]: got '{actual_cell}', expected '{expected_cell}'"
