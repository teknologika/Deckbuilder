...
class TestCrossModuleIntegration:
    ...
    def setup_method(self):
        ...
        # Mock common layouts
        layout_names = ["Title and Content", "Two Content", "Table Only", "Table with Content Above", "Comparison"]
    ...
    def test_simple_content_slide_workflow(self):
        ...
        slide_data = {"layout": "Title and Content", "title": "Test Slide Title", "content": "This is test content for the slide."}
        ...
        with patch.object(self.placeholder_resolver, "get_placeholder_by_name") as mock_get:
            ...
            title_placeholder = self.placeholder_resolver.get_placeholder_by_name(self.mock_slide, "title")
            content_placeholder = self.placeholder_resolver.get_placeholder_by_name(self.mock_slide, "content")
        ...
        with patch.object(self.content_processor, "apply_content_to_placeholder") as mock_apply:
            # Apply title
            self.content_processor.apply_content_to_placeholder(title_placeholder, slide_data["title"])

            # Apply content
            self.content_processor.apply_content_to_placeholder(content_placeholder, slide_data["content"])
...
""",
        ...
        expected_structure = [["Header 1", "Header 2", "Header 3"], ["Row 1 Col 1", "Row 1 Col 2", "Row 1 Col 3"], ["Row 2 Col 1", "Row 2 Col 2", "Row 2 Col 3"]]
        ...
        with patch.object(self.table_handler, "create_table_from_data") as mock_create:
            ...
            # Create table
            table_shape = self.table_handler.create_table_from_data(self.mock_slide, table_structure, position, size)

            # Verify table creation was called
            mock_create.assert_called_once_with(self.mock_slide, table_structure, position, size)
...
""",
        ...
        with patch.object(self.placeholder_resolver, "get_placeholder_by_name") as mock_get:
            ...
            for field_name in ["title", "content"]:
                if field_name in slide_data:
                    placeholders[field_name] = self.placeholder_resolver.get_placeholder_by_name(self.mock_slide, field_name)

            # Apply content to placeholders
            with patch.object(self.content_processor, "apply_content_to_placeholder") as mock_apply:
                for field_name, placeholder in placeholders.items():
                    self.content_processor.apply_content_to_placeholder(placeholder, slide_data[field_name])
        ...
        table_structure = self.table_handler.parse_table_structure(slide_data["table_data"])
        expected_structure = [["Product", "Price", "Stock"], ["Widget A", "$10", "50"], ["Widget B", "$15", "30"]]
        assert table_structure == expected_structure
        ...
        with patch.object(self.table_handler, "create_table_from_data") as mock_create:
            ...
            position = self.table_handler.position_table_on_slide(self.mock_slide, content_height)
    ...
    def test_error_handling_across_modules(self):
        ...
        result = self.table_handler.create_table_from_data(self.mock_slide, [], (100, 100), (200, 100))
...
"""
        ...
        expected = [["**Bold Header**", "*Italic Header*", "[Link Header](url)"], ["**Bold Data**", "*Italic Data*", "[Link Data](url2)"]]
