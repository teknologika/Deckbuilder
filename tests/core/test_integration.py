...
class TestArchitectureIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Test slide data
        self.test_slide_data = {"layout": "Title and Content", "title": "Integration Test", "content": "Testing enhanced architecture"}
    ...
    def test_slidebuilder_delegates_to_coordinator(self):
        ...
        with patch.object(self.slide_builder._coordinator, "create_slide", return_value=self.mock_slide) as mock_create:
            ...
            result = self.slide_builder.add_slide(self.mock_prs, self.test_slide_data, self.mock_content_formatter, self.mock_image_handler)

            # Verify delegation
            mock_create.assert_called_once_with(self.mock_prs, self.test_slide_data, self.mock_content_formatter, self.mock_image_handler)
    ...
    def test_enhanced_architecture_only(self):
        """Test that SlideBuilder uses only enhanced modular architecture components."""
        # Should have enhanced architecture components
        assert hasattr(self.slide_builder, "coordinator")
        assert hasattr(self.slide_builder, "layout_resolver")
        assert hasattr(self.slide_builder, "placeholder_manager")
        assert hasattr(self.slide_builder, "content_processor")
        assert hasattr(self.slide_builder, "table_handler")

        # Should NOT have legacy field_processor
        assert not hasattr(self.slide_builder, "field_processor")
    ...
    def test_error_handling_propagation(self):
        ...
        with patch.object(self.slide_builder._coordinator, "create_slide", side_effect=RuntimeError("Test error")):
            ...
            with self.assertRaises(RuntimeError) as context:
                self.slide_builder.add_slide(self.mock_prs, self.test_slide_data, self.mock_content_formatter, self.mock_image_handler)
    ...
    def test_add_slide_with_direct_mapping_compatibility(self):
        ...
        with patch.object(self.slide_builder._coordinator, "create_slide", return_value=self.mock_slide) as mock_create:
            ...
            result = self.slide_builder.add_slide_with_direct_mapping(self.mock_prs, self.test_slide_data, self.mock_content_formatter, self.mock_image_handler)
